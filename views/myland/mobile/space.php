...
if(!$otherUserId || $otherUserId == ''){
	...
	echo "<script>window.top.location.href = '".$domain."/myland/space?room=".$otherUserId."';</script>";
}
	...
	<script type="text/javascript">
		...
		window.addEventListener('message', (e)=> {
			if (e.data.info === 'onLoad') {				//페이지가 로드 됐다.
				...
				var Elecount = document.querySelector('.asmo_nav_money_box.fruit.asmo_fruit');
		...
		$(document).ready(function () {
			//document.querySelector(".navbar").style.transition = 'all .3s ease';
			document.querySelector(".navbar").classList.add('down');

			//document.querySelector("#asmo_fixed_bar").style.transition = 'all .3s ease';
			document.querySelector("#asmo_fixed_bar").classList.add('down');
