		...
		<table class="table">
			...
			?>
				...
				<?php foreach(element('list', element('comment_list', $result)) as $k => $v){ 
					...
					?>
					<tr>
						<?php if (element('is_admin', $view)) { ?><th scope="row"><input type="checkbox" name="chk_post_id[]" value="<?php echo element('post_id', $v); ?>" /></th><?php } ?>
						<td><?php echo element('num', $result); ?>(답변)</td>
						<td><a href="/post/<?php echo element('post_id', $v);?>"><?php echo html_escape(element('post_title', $v)); ?></a></td>
						<td><?php echo element('post_display_name', $v); ?></td>
