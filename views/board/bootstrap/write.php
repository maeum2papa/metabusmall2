...
<!-- asmo sh 231130 post 페이지 감싸는 div#asmo_write_wrapper, #asmo_write_wrap 생성 -->

<div id="asmo_write_wrapper">
	<div id="asmo_write_wrap">
		<div class="board">

			<!-- 디자인 상 h3 주석 처리 후 어느 페이지 소속인지 알려주는 h1 태그 생성 -->
			<!-- <h3><?php echo html_escape(element('board_name', element('board', $view))); ?> 글쓰기</h3> -->
			<h1>
				<?php
					if(element('brd_id', element('board', $view)) == '3') echo '컬래버랜드 문의하기';
				?>
				<?php
					if(element('brd_id', element('board', $view)) == '4') {
			...
			?>
				...
				<div class="form-horizontal box-table">
					...
					<!-- 제목 박스와, 문의유형 박스 감싸는 div 생성 -->
					<div class="asmo_write_title_box">
	
							<!-- 디자인 상 문의유형이 있는 form-group 주석 처리 후 제목 옆에 넣기 -->
							<?php
						...
						<div class="form-group">
							...
							<div class="col-sm-10" style="display:table;">
								<input placeholder="제목을 입력해 주세요." type="text" class="form-control" name="post_title" id="post_title" value="<?php echo set_value('post_title', element('post_title', element('post', $view))); ?>" />
					...
					<?php if (element('can_post_notice', element('post', $view)) OR element('can_post_secret', element('post', $view)) OR element('can_post_receive_email', element('post', $view))) { ?>
						...
						<?php
							if(element('brd_id', element('board', $view)) == '1' || element('brd_id', element('board', $view)) == '2') {
						?>
						<div class="form-group">
					...
					<!-- asmo sh 231220 textarea들어있는 form-group에 asmo_write_textarea 클래스 추가 -->
					<div class="form-group asmo_write_textarea">
						<div class="col-sm-12">
							...
							<!-- asmo sh 231228 수정 시 한글 깨짐 현상으로 다른 textarea 태그 삽입 -->
							...
							<textarea name="post_content" id="post_content" class="dhtmleditor"><?php echo element('post_content', element('post', $view)); ?></textarea>
...
<script type="text/javascript">
	$(document).ready(function() {
		...
		$('#asmo_write_wrapper #asmo_write_wrap .asmo_write_title_box .form-group .input-group .input option').eq(0).text('문의 유형');
	});
</script>