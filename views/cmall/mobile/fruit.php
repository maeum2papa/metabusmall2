    ...
    <div class="asmo_fruit_table_container">
        ...
        <table class="asmo_fruit_table">
            <tbody>
                ...
                ?>
                <tr>
                    <td class="asmo_fruit_date"><?php echo substr($v['log_regDt'],0,10);?></td>
                    <td class="asmo_fruit_type"><?php if($v['fru_fruit'] > 0){ echo '획득'; } else { echo '차감'; } ?></td>
                    <td class="asmo_fruit_txt"><?php echo $v['log_txt'];?></td>
                    <td class="asmo_fruit_cnt <?php if( $v['fru_fruit'] > 0 ){ ?>get<?php } ?>"><?php if( $v['fru_fruit'] > 0 ){ ?>+<?php } ?><?php echo number_format($v['fru_fruit'] );?> 개</td>
                    <td class="asmo_fruit_sum"><?php echo number_format( $v['fru_now_fruit'] );?> 개</td>
                </tr>
                <?php
                    }
                }else{
