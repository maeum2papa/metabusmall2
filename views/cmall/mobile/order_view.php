...
<div id="asmo_item_detail_wrap" class="asmo_wish_wrap asmo_order_result">
	...
	<div class="asmo_order_goods_wrap">
		<span class="asmo_order_number">주문번호 : <?php echo element('cor_id', element('data', $view)); ?></span>
					...
					<div class="order_info_box">
						<div class="prd-img">
							<a href="<?php echo cmall_item_url(element('cit_key', element('item', $result))); ?>" title="<?php echo html_escape(element('cit_name', element('item', $result))); ?>">
								<img src="<?php echo thumb_url('cmallitem', element('cit_file_1', element('item', $result))); ?>" alt="<?php echo html_escape(element('cit_name', element('item', $result))); ?>" title="<?php echo html_escape(element('cit_name', element('item', $result))); ?>" />
							</a>
						</div>
						<div class="order_option_box">
							...
							<div class="prd-price">
								...
								<div>
									<span>상품단가</span>
									<?php echo number_format(element('cit_price', element('item', $result))); ?> 개
								</div>
								<div class="prd-total">
									<span>합계</span>
									<?php echo number_format($total_price); ?><input type="hidden" name="total_price[<?php echo element('cit_id', element('item', $result)); ?>]" value="<?php echo $total_price; ?>" /> 개
								</div>
							</div>
						</div>
					</div>
	...
	<div class="credit asmo_info_container">
		...
		<div class="credit asmo_order_info_wrap asmo_payment_info asmo_order_user_info">

			<h4 class="market-title">주문자 정보</h4>
			<table class="table table-striped table-bordered table-hover">
				<tbody>
					<tr>
						<td>주문자명</td>
						<td><?php echo element('mem_realname', element('data', $view)); ?></td>
					</tr>
					<tr>
						<td>휴대폰번호</td>
						<td><?php if (element('mem_phone', element('data', $view))) {?><?php echo html_escape(element('mem_phone', element('data', $view))); ?><?php }else{?>-<?php } ?></td>
					</tr>
					<tr>
						<td>이메일</td>
						<td><?php echo element('mem_email', element('data', $view)); ?></td>
					</tr>
				</tbody>
			</table>
		</div>
		<div class="credit asmo_order_info_wrap asmo_shipping_info_box">
			<h4 class="market-title">배송지 정보</h4>
			<table class="table">
				<tbody>
					<tr class="asmo_shipping_info_tr">
						<td>(<?php if (element('cor_ship_zipcode', element('data', $view))) {?><?php echo html_escape(element('cor_ship_zipcode', element('data', $view))); ?><?php }else{?>-<?php } ?>)<br>
						<?php if (element('cor_ship_address', element('data', $view))) {?><?php echo html_escape(element('cor_ship_address', element('data', $view))); ?><?php echo html_escape(element('cor_ship_address_detail', element('data', $view))); ?><?php }else{?>-<?php } ?></td>
					</tr>
					<tr class="asmo_cor_content_tit">
						<td>주문메모</td>
					</tr>
					<tr class="asmo_cor_content_tr">
						<td>
							<?php if (element('cor_content', element('data', $view))) {?><?php echo html_escape(element('cor_content', element('data', $view))); ?><?php }else{?>-<?php } ?>
						</td>
					</tr>
				</tbody>
			</table>
