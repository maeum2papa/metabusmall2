...
$url_arr = explode('/', $url);
?>
<ul class="lmscounter-submenu btn-group">
    <li><a href="/admin/counter/lmscounter" class="btn btn-sm btn-default <?php if(!$url_arr[3]){ echo 'btn-warning'; } ?>">시간대별 학습통계</a></li>
	<li><a href="/admin/counter/lmscounter/apply" class="btn btn-sm btn-default <?php if($url_arr[3] == 'apply'){ echo 'btn-warning'; } ?>">학습 신청 통계</a></li>
	<li><a href="/admin/counter/lmscounter/cert" class="btn btn-sm btn-default <?php if($url_arr[3] == 'cert'){ echo 'btn-warning'; } ?>">교육 이수증</a></li>
</ul>
<div class="box">
    ...
    <div class="box-table">
                    ...
                    <tr>
                        ...
                        <td>
                            <?php if(element('mp_endYn', $result) == 'y'){ ?>
                                <a href="/admin/counter/lmscounter/cert_export/<?php echo element('mp_sno', $result);?>">출력</a>
                            <?php } else { ?>
                                미출력
                            <?php } ?>
                        </td>
        ...
        <div class="box-info">
			<div class="btn-group pull-right" role="group" aria-label="...">
                <a href="/admin/counter/lmscounter/cert_export_all/<?php echo $this->member->item('company_idx');?>?<?php echo $this->input->server('QUERY_STRING', null, '');?>" class="btn btn-outline btn-default btn-sm">수료증 전체 다운로드</a>
...
$('#cca_id_fir').change(function(){
    if($(this).val() != ''){
        $.ajax({
            method: "POST",
            url: "/admin/counter/lmscounter/getCcaChild",
            data: {
                mode: 'getCcaChild',
                cca_parent: $(this).val(),
