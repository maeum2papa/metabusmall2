				...
				if ($p && is_array($p)) {
					foreach ($p as $result) {
						...
						if ( ! element(element('cca_id', $result), $data)) {
							$return .= '					<button class="btn btn-danger btn-xs btn-one-delete" data-one-delete-url = "' . admin_url('lms/curriculum/delete/' . element('cca_id', $result)) . '"><span class="glyphicon glyphicon-trash"></span></button>';
						}
						...
						$return .= '<input type="hidden" name="cca_id"	value="' . element('cca_id', $result) . '" />
															...
															<div class="form-group" style="margin-left:0;">
																커리큘럼 <input type="text" class="form-control" name="cca_value" value="' . html_escape(element('cca_value', $result)) . '" />
			...
			?>
				...
				<div class="form-group">
					...
					<div class="col-sm-8 form-inline">
						...
						<input type="text" name="cca_value" class="form-control" value="" placeholder="카테고리명 영문" />
