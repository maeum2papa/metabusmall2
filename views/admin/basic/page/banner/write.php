			...
			<div class="form-group">
				...
				<div class="col-sm-10">
					...
					if (element('ban_image', element('data', $view))) {
						$imgUrl = banner_image_url(element('ban_image', element('data', $view)));
						if(mb_substr($imgUrl, -3, 3) == 'svg'){
							$svgBg = "style='background:#000;'";
						}
					?>
						<img src="<?php echo banner_image_url(element('ban_image', element('data', $view))); ?>" <?php echo $svgBg; ?> height="150" alt="배너 이미지" title="배너 이미지" />
						<label for="ban_image_del">
							<input type="checkbox" name="ban_image_del" id="ban_image_del" value="n" /> 삭제
						</label>
					<?php
					}
					?>
...
//<![CDATA[
$(function() {
	$('#ban_image_del').on('click', function(){
		if($('#ban_image_del').is(':checked') == true){
			$('#ban_image_del').val('y');
		} else {
			$('#ban_image_del').val('n');
		}
