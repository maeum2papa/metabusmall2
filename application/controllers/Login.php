...
class Login extends CB_Controller
{
	...
	public function index()
	{
		...
		//231222 test 로그아웃
//		$wheres = array(
//			'mem_id' => $this->input->post('mem_userid'),
//		);
//		$this->load->model(array('Autologin_model'));
//		$this->Autologin_model->delete_where($wheres);
//
...
//		$this->session->sess_destroy();
		...
		if ($this->form_validation->run() === false) {
            ...
            $select = 'cb_member.mem_id, cb_member.mem_userid, cb_member.company_idx, (select count(*) from cb_member_group_member where mem_id = cb_member.mem_id and mgr_id = 2) as mem_admin_flag ';

			if ($use_login_account === 'both') {
				$userinfo = $this->Member_model->get_by_both($this->input->post('mem_userid'), $select);
			} elseif ($use_login_account === 'email') {
				$userinfo = $this->Member_model->get_by_email($this->input->post('mem_userid'), $select);
			} else {
				$userinfo = $this->Member_model->get_by_userid($this->input->post('mem_userid'), $select);
			}
            ...
            $this->session->set_userdata('mem_userid', element('mem_userid', $userinfo));
            $this->session->set_userdata('company_idx', element('company_idx', $userinfo));
            $this->session->set_userdata('mem_admin_flag', element('mem_admin_flag', $userinfo));
			...
			//회원 테이블에 라스트로그인 저장
			$time_chk = time();
			$sql = "UPDATE cb_member SET mem_login_chk = '".$time_chk."' WHERE mem_id = '".element('mem_id', $userinfo)."'";
            $this->db->query($sql);
			$_SESSION['seum_login_chk'] = $time_chk;
			//231114 로그인 뒤  이동할 주소 세팅 asmo yjb
			$q = "select company_code from cb_company_info where company_idx = '".element('company_idx', $userinfo)."'";
			$r = $this->db->query($q);
...
//			}
	...
	public function logout()
	{
		...
		// 접속시간을 구하기 위해 로그아웃 시간 저장
		$q = "SELECT mll_id FROM cb_member_login_log WHERE mem_id = '".$this->member->item('mem_id')."' and mll_success = 1 order by mll_id desc limit 0, 1";
		$r = $this->db->query($q);
		$mll = $r->row_array();
		$time_chk = date('Y-m-d H:i:s');
		$sql = "UPDATE cb_member_login_log SET mll_outtime = '".$time_chk."' WHERE mll_id = '".$mll['mll_id']."'";
		$this->db->query($sql);
	...
	public function logout_seum($mem_id)
	{
		// 이벤트 라이브러리를 로딩합니다
		$eventname = 'event_logout_index';
		$this->load->event($eventname);
		...
		// 접속시간을 구하기 위해 로그아웃 시간 저장
		$q = "SELECT mll_id FROM cb_member_login_log WHERE mem_id = '".$mem_id."' and mll_success = 1 order by mll_id desc limit 0, 1";
		$r = $this->db->query($q);
		$mll = $r->row_array();
		$time_chk = date('Y-m-d H:i:s');
		$sql = "UPDATE cb_member_login_log SET mll_outtime = '".$time_chk."' WHERE mll_id = '".$mll['mll_id']."'";
