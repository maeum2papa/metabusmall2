...
class Verify extends CB_Controller
{
	...
	public function confirmemail()
	{
		...
		if ( ! element('mae_id', $result)) {
			...
			$select = 'mem_id, mem_userid, mem_denied, mem_email_cert, mem_nickname, mem_email';
			...
			if ( ! element('mem_id', $dbmember)) {
				...
				$this->session->set_userdata('mem_nickname', element('mem_nickname', $dbmember));
				$this->session->set_userdata('mem_email', element('mem_email', $dbmember));
			}
		}
	...
	public function resetpassword()
	{
		...
		if ( ! element('mae_id', $result)) {
			...
			$view['view']['mem_nickname'] = element('mem_nickname', $dbmember);
		...
		if ($this->form_validation->run() === false) {
			...
			if (empty($view['view']['error_message'])) {
				...
				$q = "select mem_userid from cb_member where mem_id = '".element('mem_id', $result)."'";
				$r = $this->db->query($q);
				$seum_id = (array) $r->row();
				
				$this->member->update_login_log(element('mem_id', $result), $seum_id[mem_userid], 1, '패스워드 변경 후 로그인 성공');
