 ...
 */
class Business_process extends CB_Controller
{
	 ...
	 * 페이지 이동시 필요한 정보입니다
	 */
	public $pagedir = 'lms/business_process';
	...
	public function index()
	{
		...
		$view['view']['sort'] = array(
			'p_sno' => $param->sort('lms_process.p_sno', 'asc'),
			'plan_idx' => $param->sort('lms_process.plan_idx', 'asc'),
			'p_regDt' => $param->sort('lms_process.p_regDt', 'desc'),
		);
		...
		$where[] = "(cb_lms_process.p_viewYn = 'y' or (cb_lms_process.p_viewYn = 's' and cb_lms_process.p_sdate <= ".$today." and cb_lms_process.p_edate >= ".$today."))";
		$_GET['skeyword'] = trim($_GET['skeyword']);
		if ($_GET['skeyword'] != ""){	
			$view['view']['skeyword'] = $_GET['skeyword'];
			$view['view']['sfield'] = $_GET['sfield'];
			$where[] = "cb_lms_process.".$_GET['sfield']." like '%".$_GET['skeyword']."%'";
		}
		if ($_GET['plan_idx']) {	
			$view['view']['plan_idx'] = $_GET['plan_idx'];
			$where[] = "cb_lms_process.plan_idx = '".$_GET['plan_idx']."'";
		}
		if ($_GET['p_curriYn']) {	
			$view['view']['p_curriYn'] = $_GET['p_curriYn'];
			$where[] = "cb_lms_process.p_curriYn = '".$_GET['p_curriYn']."'";
		}
		
		$sql_limit = $offset.",".$per_page;
		//$result = $this->{$this->modelname}->get_business_list($per_page, $offset, $where, '', $findex, $forder, $sfield, $skeyword);
		//debug($result);
		//조건절 복잡하여 포기
		$q = "select cb_lms_process.*, cb_plan.plan_name from cb_lms_process left join cb_plan on cb_lms_process.plan_idx = cb_plan.plan_idx where ".implode(" and ",$where)." order by ".$findex." ".$forder." limit ".$sql_limit;
		//debug($q);
		$r = $this->db->query($q);
		$result['list'] = $r->result_array(); 
		
		$q = "select count(cb_lms_process.p_sno) as cnt from cb_lms_process left join cb_plan on cb_lms_process.plan_idx = cb_plan.plan_idx where ".implode(" and ",$where);
		$r = $this->db->query($q);
		$tot_cnt = (array) $r->row();
		$result['total_rows'] = $tot_cnt[cnt];
		//debug($result);
		...
		if (element('list', $result)) {
			foreach (element('list', $result) as $key => $val) {
				...
				//노출가능여부
				$result['list'][$key]['busi_exYn'] = business_exYn($result['list'][$key]['p_sno'],$this->member->item('company_idx'));
				//카테고리
		...
		$view['view']['list_ex_url'] = admin_url($this->pagedir . '/listex/?' . $param->output());
		$view['view']['list_unex_url'] = admin_url($this->pagedir . '/listunex/?' . $param->output());
...
//		$this->load->event($eventname);
		...
		if ($this->input->post('chk') && is_array($this->input->post('chk'))) {
			foreach ($this->input->post('chk') as $val) {
				if ($val) {
					$q = "select ce_sno from cb_lms_process_company_exposure where company_idx = '".$this->member->item('company_idx')."' and p_sno = '".$val."'";
					$r = $this->db->query($q);
					$exist = (array) $r->row();
					if(!$exist[ce_sno]){
						$param = array(
						...
						$this->db->insert('cb_lms_process_company_exposure', $param);
					}
		...
		if ($this->input->post('chk') && is_array($this->input->post('chk'))) {
			foreach ($this->input->post('chk') as $val) {
				if ($val) {
					...
					$exist = (array) $r->row();
					if($exist[ce_sno]){
						$delete_array = array('ce_sno' => $exist[ce_sno]);
						$result = $this->db->delete('cb_lms_process_company_exposure', $delete_array);    
					}
				}
			}
		}
