 ...
 */
class Masterstatcnt extends CB_Controller
{
	 ...
	 * 페이지 이동시 필요한 정보입니다
	 */
	public $pagedir = 'counter/masterstatcnt';
	...
	public function index($export = '')
	{
        ...
        if($this->input->get('company_idx') && empty($this->input->get('company_idx') === false)){
            $sWhere .= " and company_idx = ".$this->input->get('company_idx');
        }
        ...
        $sWhere3 = "";
        if($this->input->get('company_idx') && empty($this->input->get('company_idx') === false)){
            $sWhere3 .= " and member.company_idx = ".$this->input->get('company_idx');
        }

		if($this->input->get('ymd') == 'day'){
			$sql = "SELECT mll_useragent FROM cb_member_login_log as a 
			left join cb_member as member on member.mem_id = a.mem_id 
			WHERE 1 ".$sWhere3." and a.mll_datetime >= '".date('Y-m-d')." 00:00:00' and a.mll_datetime <= '".date('Y-m-d')." 23:59:59' and a.mll_success = 1 ORDER BY mll_datetime desc";
			$r = $this->db->query($sql);
			$result2 = $r->result_array();
		} else if($this->input->get('ymd') == 'month'){
			$sql = "SELECT mll_useragent FROM cb_member_login_log as a 
			left join cb_member as member on member.mem_id = a.mem_id 
			WHERE 1 ".$sWhere3." and a.mll_datetime >= '".date('Y-m')."-01 00:00:00' and a.mll_datetime <= '".date('Y-m')."-31 23:59:59' and a.mll_success = 1 ORDER BY mll_datetime desc";
			$r = $this->db->query($sql);
			$result2 = $r->result_array();
		} else if($this->input->get('ymd') == 'year'){
			$sql = "SELECT mll_useragent FROM cb_member_login_log as a 
			left join cb_member as member on member.mem_id = a.mem_id 
			WHERE 1 ".$sWhere3." and a.mll_datetime >= '".date('Y')."-01-01 00:00:00' and a.mll_datetime <= '".date('Y')."-12-31 23:59:59' and a.mll_success = 1 ORDER BY mll_datetime desc";
			$r = $this->db->query($sql);
			$result2 = $r->result_array();
		} else {
			$sql = "SELECT mll_useragent FROM cb_member_login_log as a 
		...
		if ($result2 && is_array($result2)) {
			foreach ($result2 as $key => $value) {
				$userAgent = get_useragent_info($value['mll_useragent']);
				$s = $userAgent['os'];
				if (empty($s)) {
					$s = '-';
				}
	...
	public function users($export = '')
	{
		...
		left join cb_member as b on b.mem_id = a.mem_id where a.mll_datetime >= '".$start_date."' and a.mll_datetime <= '".$end_date."' ".$sWhere." and 
		...
		foreach($result as $key => $value){
			...
			left join cb_member as b on b.mem_id = a.mem_id where a.mll_datetime like '".$like."' ".$sWhere." and a.mll_success = 1";
			$r = $this->db->query($q);
			$res1 = $r->row_array();
			...
			$q = "select count(mll_id) as visit_count from cb_member_login_log as a 
			left join cb_member as b on b.mem_id = a.mem_id where a.mll_datetime <= '".$datetime."' ".$sWhere." and a.mll_success = 1";
			$r = $this->db->query($q);
			$res2 = $r->row_array();
	...
	public function devices($export = '')
	{
		...
		if($this->input->get('deviceFl') == 'mobile'){
			...
			where a.mll_datetime >= '".$start_date."' and a.mll_datetime <= '".$end_date."' ".$sWhere." and a.mll_success = 1 
			...
			where a.mll_datetime <= '".$end_date."' ".$sWhere." and a.mll_success = 1 
		...
		$sql = "select ".$select." from cb_member_login_log as a left join cb_member as b on b.mem_id = a.mem_id where a.mll_success = 1 ".$sWhere." group by 1";
		$r = $this->db->query($sql);
		$result = $r->row_array();
		if($this->input->get('deviceFl') == 'mobile'){
			$data = array(
