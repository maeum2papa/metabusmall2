 ...
 */
class Masterlmscnt extends CB_Controller
{
    /**
	 * 관리자 페이지 상의 현재 디렉토리입니다
	 * 페이지 이동시 필요한 정보입니다
	 */
	public $pagedir = 'counter/masterlmscnt';
	...
	public function index($export = '')
	{
        ...
        // 기업별 필터
		$sql = "select company_idx, company_name from cb_company_info where state = 'use'";
		$r = $this->db->query($sql);
		$rst = $r->result_array();

		$view['view']['filter'] = $rst;

        $sWhere = ""; 
        if($this->input->get('company_idx') && empty($this->input->get('company_idx') === false)){
            $sWhere .= " and a.company_idx = ".$this->input->get('company_idx');
        }

		if($this->input->get('sfield') && $this->input->get('skeyword')){
			$sWhere .= " and a.".$this->input->get('sfield')." like '%".$this->input->get('skeyword')."%' ";
		}
        ...
        $sWhere2 = ""; 
        if($this->input->get('company_idx') && empty($this->input->get('company_idx') === false)){
            $sWhere2 .= " and b.company_idx = ".$this->input->get('company_idx');
        }

		if($this->input->get('sfield') && $this->input->get('skeyword')){
			$sWhere2 .= " and b.".$this->input->get('sfield')." like '%".$this->input->get('skeyword')."%' ";
		}
		...
		if($this->input->get('ymd') == 'day'){
			...
			WHERE 1 and a.mll_datetime >= '".$start_date." 00:00:00' and mll_datetime <= '".$end_date." 23:59:59' ".$sWhere2." group by 1 order by mll_datetime asc";
	...
	public function apply($export = '')
	{
        ...
        foreach($result as $k => $v){
            ...
            // 신청자 수
            $sql = "select count(*) as cnt from cb_my_process as a left join cb_member as b on b.mem_id = a.mem_id where 1 and a.p_sno = '".$v['p_sno']."' ".$sWhere2;
            $r = $this->db->query($sql);
            $rowdata = $r->row_array();
            $result[$k]['plan_apply_cnt'] = $rowdata['cnt'];

            // 수료자 수
            $sql = "select count(*) as cnt from cb_my_process as a left join cb_member as b on b.mem_id = a.mem_id where 1 and a.p_sno = '".$v['p_sno']."' and a.mp_endYn = 'y' ".$sWhere2;
            $r = $this->db->query($sql);
            $rowdata = $r->row_array();
        ...
        $sWhere3 = "";
            if($this->input->get('company_idx') && empty($this->input->get('company_idx') === false)){
                $sWhere3 .= " and b.company_idx = ".$this->input->get('company_idx');
            }
		$start_date = date('Y-m-d');
		$end_date = date('Y-m-d');
		if($this->input->get('ymd') == 'day'){
            $sql = "select count(*) as cnt from cb_my_process as a left join cb_member as b on b.mem_id = a.mem_id 
            where 1 ".$sWhere3." and a.mp_regDt >= '".$start_date." 00:00:00' and a.mp_regDt <= '".$end_date." 23:59:59'";
            $r = $this->db->query($sql);
            $rowdata = $r->row_array();
            ...
            $sql = "select count(*) as cnt from cb_my_process as a left join cb_member as b on b.mem_id = a.mem_id 
            where 1 ".$sWhere3." and a.mp_regDt >= '".$ex_start_date." 00:00:00' and a.mp_regDt <= '".$ex_end_date." 23:59:59'";
            $r = $this->db->query($sql);
            $rowdata = $r->row_array();
			...
			$sql = "select count(*) as cnt from cb_my_process as a left join cb_member as b on b.mem_id = a.mem_id 
            where 1 ".$sWhere3." and mp_regDt >= '".$start_date." 00:00:00' and mp_regDt <= '".$end_date." 23:59:59'";
            $r = $this->db->query($sql);
            $rowdata = $r->row_array();
            $result2['cnt'] = $rowdata['cnt'];
            ...
            $ex_end_date = date('Y-m-d', strtotime('-1 day'));
            $sql = "select count(*) as cnt from cb_my_process as a left join cb_member as b on b.mem_id = a.mem_id 
            where 1 ".$sWhere3." and mp_regDt >= '".$ex_start_date." 00:00:00' and mp_regDt <= '".$ex_end_date." 23:59:59'";
            $r = $this->db->query($sql);
            $rowdata = $r->row_array();
	...
	public function apply_detail($p_sno = 0)
	{
        ...
        $sWhere = "";
        if($this->input->get('company_idx') && empty($this->input->get('company_idx') === false)){
            $sWhere .= " and b.company_idx = ".$this->input->get('company_idx');
        }
	...
	public function cert($export = '')
	{
        ...
        if($this->input->get('mem_div')){
            $sWhere .= " and b.mem_div like '%".$this->input->get('mem_div')."%' ";
        }

        // 회원 검색
        if($this->input->get('mem_username')){
            $sWhere .= " and (b.mem_username like '%".$this->input->get('mem_username')."%' or b.mem_nickname like '%".$this->input->get('mem_username')."%') ";
        }
		...
		if(!$list_ymd){
			if($list_start_date && $list_end_date){
				$sWhere .= " and a.mp_endDt >= '".$list_start_date." 00:00:00' and a.mp_endDt <= '".$list_end_date." 23:59:59' ";
				$view['view']['list_ymd'] = '';
			}
		} else {
			if($list_ymd == 'day'){
				$view['view']['list_start_date'] = date('Y-m-d');
				$view['view']['list_end_date'] = date('Y-m-d');
				$sWhere .= " and a.mp_endDt >= '".$view['view']['list_start_date']." 00:00:00' and a.mp_endDt <= '".$view['view']['list_end_date']." 23:59:59' ";
			} else if($list_ymd == '7day'){
				$view['view']['list_start_date'] = date('Y-m-d', strtotime('-7 day'));
				$view['view']['list_end_date'] = date('Y-m-d');
				$sWhere .= " and a.mp_endDt >= '".$view['view']['list_start_date']." 00:00:00' and a.mp_endDt <= '".$view['view']['list_end_date']." 23:59:59' ";
        ...
        if($this->input->get('cca_id_fir')){
            if($this->input->get('cca_id_sec')){
                $sWhere .= " and e.cca_id = ".$this->input->get('cca_id_sec');
            } else {
                $sWhere .= " and e.cca_parent = ".$this->input->get('cca_id_fir');
            }
        }

        // 과정명 검색
        if($this->input->get('list_skeyword')){
			$sWhere .= " and c.p_title like '%".$this->input->get('list_skeyword')."%' ";
		}
        ...
        where 1 ".$sWhere." order by a.mp_endDt desc, e.cca_id asc, e.cca_order asc";
		$qry = $this->db->query($sql);
		$total_result = $qry->result_array();
		$total_rows = count($total_result);
	 ...
	 * 교육 이수증 출력 다건
	 */
    public function cert_export_all()
    {
        $view = array();
		$view['view'] = array();
        ...
        if($this->input->get('cca_id_fir')){
            if($this->input->get('cca_id_sec')){
                $sWhere .= " and f.cca_id = ".$this->input->get('cca_id_sec');
            } else {
                $sWhere .= " and f.cca_parent = ".$this->input->get('cca_id_fir');
            }
        }

        // 과정명 검색
        if($this->input->get('list_skeyword')){
			$sWhere .= " and d.p_title like '%".$this->input->get('list_skeyword')."%' ";
		}
        ...
        where 1 ".$sWhere." and a.mp_endYn = 'y' order by a.mp_endDt desc, f.cca_id asc, f.cca_order asc";
        $r = $this->db->query($sql);
