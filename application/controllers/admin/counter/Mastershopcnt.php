 ...
 */
class Mastershopcnt extends CB_Controller
{
    /**
	 * 관리자 페이지 상의 현재 디렉토리입니다
	 * 페이지 이동시 필요한 정보입니다
	 */
	public $pagedir = 'counter/mastershopcnt';
	...
	public function index($export = '')
	{
        ...
        if($this->input->get('company_idx') && empty($this->input->get('company_idx') === false)){
            $sWhere .= " and e.company_idx = ".$this->input->get('company_idx');
        }
		...
		foreach($result as $k => $v){
			...
			if($this->input->get('company_idx') && empty($this->input->get('company_idx') === false)){
				$sWhere2 .= " and c.company_idx = ".$this->input->get('company_idx');
			}
		...
		if ($export === 'excel') {
			...
			header('Content-Disposition: attachment; filename=판매순위_' . cdate('Y_m_d') . '.xls');
	...
	public function order($export = '')
	{
		...
		if($this->input->get('company_idx') && empty($this->input->get('company_idx') === false)){
            $sWhere .= " and c.company_idx = ".$this->input->get('company_idx');
        }
		 ...
		 * 페이지네이션을 생성합니다
		 */
		$config['base_url'] = admin_url($this->pagedir) . '/order?' . $param->replace('page');
		$config['total_rows'] = $total_rows;
		$config['per_page'] = $per_page;
		$this->pagination->initialize($config);
		$view['view']['paging'] = $this->pagination->create_links();
		...
		if ($export === 'excel') {
			...
			header('Content-type: application/vnd.ms-excel');
			header('Content-Disposition: attachment; filename=주문통계_' . cdate('Y_m_d') . '.xls');
