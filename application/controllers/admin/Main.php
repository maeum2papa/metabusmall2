...
class Main extends CB_Controller
{
    ...
    {
        $mem_admin_flag = $this->session->userdata['mem_admin_flag'];
        //$mem_admin_flag = 1;

        if($mem_admin_flag == 0) $this->admin_index();
        else $this->company_index();
    }
	...
	public function admin_index()
	{
		...
		$view['layout'] = $this->managelayout->admin($layoutconfig);
		$this->data = $view;
    ...
    public function company_index()
    {
         ...
         * 어드민 레이아웃을 정의합니다
         */
        $layoutconfig = array('layout' => 'layout', 'skin' => 'cmain');
        $view['layout'] = $this->managelayout->admin($layoutconfig, $this->cbconfig->get_device_view_type());
        $this->data = $view;
        $this->layout = element('layout_skin_file', element('layout', $view));
        $this->view = element('view_skin_file', element('layout', $view));
