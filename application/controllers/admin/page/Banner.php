		...
		if (element('list', $result)) {
			foreach (element('list', $result) as $key => $val) {
				if (element('ban_image', $val)) {
					$result['list'][$key]['thumb_url'] = thumb_url('banner', element('ban_image', $val));
					$result['list'][$key]['thumb_ext'] = mb_substr(thumb_url('banner', element('ban_image', $val)), -3, 3);
				}
	...
	public function write($pid = 0)
	{
		...
		if ($form_validation) {
			...
			if (isset($_FILES) && isset($_FILES['ban_image']) && isset($_FILES['ban_image']['name']) && $_FILES['ban_image']['name']) {
				...
				$uploadconfig['allowed_types'] = 'jpg|jpeg|png|gif|image/svg+xml|svg';
		...
		if ($form_validation === false OR $file_error !== '') {
			...
			//var_dump($this->input->post('ban_image_del'));exit;
			// if ($this->input->post('ban_image_del') !== 'n') {
