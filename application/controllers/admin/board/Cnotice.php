...
/**
 * 관리자>게시판설정>공지사항(기업) controller 입니다.
 */
class CNotice extends CB_Controller
{
	...
	function __construct()
	{
		 ...
		 * 라이브러리를 로딩합니다
		 */
		$this->load->library(array('pagination', 'querystring', 'accesslevel', 'email', 'notelib', 'point', 'imagelib', 'videoplayer'));
	}
	...
	public function index()
	{
		...
		$result = $this->{$this->modelname}
			->get_admin_cnotice_list($per_page, $offset, $where, '', $findex, $forder, $sfield, $skeyword, $company_idx);
	 ...
	 * 게시물 작성 페이지입니다
	 */
	public function write($brd_key = 'cnotice')
	{
		// 이벤트 라이브러리를 로딩합니다
		$eventname = 'event_admin_board_write_write';
		$this->load->event($eventname);
	...
	public function modify($post_id = 0)
	{
		...
		if ($form_validation === false OR $file_error) {
			...
			$redirecturl = '/admin/board/cnotice/';
