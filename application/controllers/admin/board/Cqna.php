...
/**
 * 관리자>게시판설정>문의게시판 controller 입니다.
 */
class Cqna extends CB_Controller
{
	...
	public function index()
	{
		 ...
		 */
		$this->{$this->modelname}->allow_search_field = array('post_id', 'post_title', 'post_content', 'mem_id', 'post_username', 'post_nickname', 'post_email', 'post_homepage', 'post_datetime', 'post_ip', 'post_device', 'post_reply_chk'); // 검색이 가능한 필드
		...
		$mem_userid = element('mem_userid', $userdata);
		
		$company_result = $this->{$this->modelname}->get_company_idx($mem_userid);
		$company_idx = $company_result['list'][0]['company_idx'];
		
		$result = $this->{$this->modelname}
			->get_admin_cqna_list($per_page, $offset, $where, '', $findex, $forder, $sfield, $skeyword, $company_idx);
		...
		if (element('list', $result)) {
			foreach (element('list', $result) as $key => $val) {
				...
				$result['list'][$key]['company_info'] = $this->{$this->modelname}->get_company_info(element('mem_id', $val));
				$result['list'][$key]['company_name'] = $result['list'][$key]['company_info']['list'][0]['company_name'];

				$result['list'][$key]['comment_list'] = $this->{$this->modelname}->get_comment_list(element('post_num', $val));
				foreach(element('list', $result['list'][$key]['comment_list']) as $key2 => $val2){
					$result['list'][$key]['comment_list']['list'][$key2]['post_display_name'] = display_username(
						element('post_userid', $val2),
						element('post_nickname', $val2)
					);
					$result['list'][$key]['comment_list']['list'][$key2]['board'] = $board2 = $this->board->item_all(element('brd_id', $val2));
					if ($board2) {
						$result['list'][$key]['comment_list']['list'][$key2]['boardurl'] = board_url(element('brd_key', $board2));
						$result['list'][$key]['comment_list']['list'][$key2]['posturl'] = post_url(element('brd_key', $board2), element('post_id', $val2));
					}
					$result['list'][$key]['comment_list']['list'][$key2]['category'] = '';
					if(element('post_category', $val2)){
						$result['list'][$key]['comment_list']['list'][$key2]['category'] = $this->Board_category_model->get_category_info(element('brd_id', $val2), element('post_category', $val2));
					}
					if (element('post_image', $val2)) {
						$imagewhere = array(
							'post_id' => element('post_id', $val2),
							'pfi_is_image' => 1,
						);
						$file2 = $this->Post_file_model->get_one('', '', $imagewhere, '', '', 'pfi_id', 'ASC');
						$result['list'][$key]['comment_list']['list'][$key2]['thumb_url'] = thumb_url('post', element('pfi_filename', $file2), 80);
					} else {
						$result['list'][$key]['comment_list']['list'][$key2]['thumb_url'] = get_post_image_url(element('post_content', $val2), 80);
					}

					$extravars = element('extravars', $board);
					$form = json_decode($extravars, true);

					$post2['extravars'] = $this->Post_extra_vars_model->get_all_meta(element('post_id', $val2));

					$extra_content = array();
					if($form && is_array($form)){
						foreach($form as $kkk => $vvv){
							if(!element('use', $vvv)){
								continue;
							}

							$item = element(element('field_name', $vvv), element('extravars', $post2));
							$extra_content['field_name'] = element('field_name', $vvv);
							$extra_content['display_name'] = element('display_name', $vvv);
							$extra_content['output'] = $item;
						}
						$result['list'][$key]['comment_list']['list'][$key2]['extra_content'] = $extra_content;
					}
				...
				$post['extravars'] = $this->Post_extra_vars_model->get_all_meta(element('post_id', $val));
				...
				if($form && is_array($form)){
					foreach($form as $kk => $vv){
						...
						$item = element(element('field_name', $vv), element('extravars', $post));
						$extra_content['field_name'] = element('field_name', $vv);
						$extra_content['display_name'] = element('display_name', $vv);
						$extra_content['output'] = $item;
					}
		 ...
		 */
		$search_option = array('post_title' => '제목', 'post_content' => '내용', 'post_username' => '실명', 'post_nickname' => '닉네임', 'post_email' => '이메일', 'post_homepage' => '홈페이지', 'post_datetime' => '작성일', 'post_ip' => 'IP', 'post_reply_chk' => '처리상태');
	...
	/**
	 * 목록 페이지에서 상세보기를 하는 경우 실행되는 메소드입니다
	 */
	public function post($post_id = 0, $print = false)
	{
		// 이벤트 라이브러리를 로딩합니다
		$eventname = 'event_admin_board_qna_post';
		$this->load->event($eventname);
        ...
        $post['mem_company'] = $this->Post_model->get_mem_company($post_id);
		$view['view']['post'] = $post;
		...
		if($form && is_array($form)){
			...
			$view['view']['post']['extra_content'] = $extra_content;
		}

		$origin = $this->Post_model->get_one(element('post_id', $post));

		$view['view']['comment_list'] = array();
		$view['view']['comment_list'] = $this->{$this->modelname}->get_comment_list(element('post_num', $view['view']['post']));
		foreach(element('list', $view['view']['comment_list']) as $key => $val){
			$view['view']['comment_list']['list'][$key]['post_display_name'] = display_username(
				element('post_userid', $val),
				element('post_nickname', $val)
			);
			$view['view']['comment_list']['list'][$key]['board'] = $board2 = $this->board->item_all(element('brd_id', $val));
			if ($board2) {
				$view['view']['comment_list']['list'][$key]['boardurl'] = board_url(element('brd_key', $board2));
				$view['view']['comment_list']['list'][$key]['posturl'] = post_url(element('brd_key', $board2), element('post_id', $val));
			}
			$view['view']['comment_list']['list'][$key]['category'] = '';
			if(element('post_category', $val)){
				$view['view']['comment_list']['list'][$key]['category'] = $this->Board_category_model->get_category_info(element('brd_id', $val), element('post_category', $val));
			}
			if (element('post_image', $val)) {
				$imagewhere = array(
					'post_id' => element('post_id', $val),
				...
				$file = $this->Post_file_model->get_one('', '', $imagewhere, '', '', 'pfi_id', 'ASC');
				$view['view']['comment_list']['list'][$key]['thumb_url'] = thumb_url('post', element('pfi_filename', $file), 80);
			} else {
				$view['view']['comment_list']['list'][$key]['thumb_url'] = get_post_image_url(element('post_content', $val), 80);
			}

			$link_player2 = '';
			$view['view']['comment_list']['list'][$key]['link'] = $link2 = array();
			...
			if (element('post_link_count', $val)) {
				...
				$view['view']['comment_list']['list'][$key]['link'] = $link2 = $this->Post_link_model
					->get('', '', $linkwhere, '', '', 'pln_id', 'ASC');
				if ($link2 && is_array($link2)) {
					foreach ($link2 as $key2 => $value2) {
						$view['view']['comment_list']['list'][$key]['link'][$key2]['link_link'] = site_url('postact/link/' . element('pln_id', $value2));
					}
				}
			}
			$view['view']['comment_list']['list'][$key]['link_count'] = $link_count2 = count($link2);
			...
			if (element('post_file', $val) OR element('post_image', $val)) {
				...
				$view['view']['comment_list']['list'][$key]['file'] = $file2 = $this->Post_file_model
					->get('', '', $filewhere, '', '', 'pfi_id', 'ASC');
				$view['view']['comment_list']['list'][$key]['file_download'] = array();
				$view['view']['comment_list']['list'][$key]['file_image'] = array();

				$play_extension = array('acc', 'flv', 'f4a', 'f4v', 'mov', 'mp3', 'mp4', 'm4a', 'm4v', 'oga', 'ogg', 'rss', 'webm');

				if ($file2 && is_array($file2)) {
					foreach ($file2 as $key2 => $value2) {
						if (element('pfi_is_image', $value2)) {
							$value2['origin_image_url'] = site_url(config_item('uploads_dir') . '/post/' . element('pfi_filename', $value2));
							$value2['thumb_image_url'] = thumb_url('post', element('pfi_filename', $value2), $image_width);
							$view['view']['comment_list']['list'][$key]['file_image'][] = $value2;
						} else {
							$value2['download_link'] = site_url('postact/download/' . element('pfi_id', $value2));
							$view['view']['comment_list']['list'][$key]['file_download'][] = $value2;
							if (element('use_autoplay', $board) && in_array(element('pfi_type', $value2), $play_extension)) {
								$file_player2 .= $this->videoplayer->get_jwplayer(site_url(config_item('uploads_dir') . '/post/' . element('pfi_filename', $value2)), $image_width);
							}
						}
					}
				}
				$view['view']['comment_list']['list'][$key]['file_count'] = count($file);
				$view['view']['comment_list']['list'][$key]['file_download_count'] = count($view['view']['file_download']);
				$view['view']['comment_list']['list'][$key]['file_image_count'] = count($view['view']['file_image']);
			}
		...
		$view['view']['board'] = $board = $this->board->item_all(element('brd_id', $view['view']['post']));
		...
		if ($form_validation === false OR $file_error) {
			...
			$this->Post_model->update_reply_chk($post_id);
			...
			/**
			 * 게시물의 신규입력 또는 수정작업이 끝난 후 리스트 페이지로 이동합니다
			 */
			$redirecturl = '/admin/board/cqna/';
	...
	{
		// 이벤트 라이브러리를 로딩합니다
		$eventname = 'event_admin_board_write_modify';
		$this->load->event($eventname);
