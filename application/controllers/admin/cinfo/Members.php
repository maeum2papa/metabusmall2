...
class Members extends CB_Controller
{
	 ...
	 * 페이지 이동시 필요한 정보입니다
	 */
	public $pagedir = 'cinfo/members';

	/**
	 * 모델을 로딩합니다
	 */
	protected $models = array('Company_info','Member_meta', 'Member_group', 'Member_group_member', 'Member_nickname', 'Member_extra_vars', 'Member_userid', 'Social_meta', 'Post');
	...
	public function index()
	{
		 ...
		 */
		$this->{$this->modelname}->allow_search_field = array('company_name','mem_id', 'mem_userid', 'mem_email', 'mem_username', 'mem_nickname', 'mem_register_datetime', 'mem_register_ip', 'mem_lastlogin_datetime', 'mem_lastlogin_ip', 'mem_is_admin'); // 검색이 가능한 필드
		$this->{$this->modelname}->search_field_equal = array('mem_id', 'mem_level', 'mem_is_admin'); // 검색중 like 가 아닌 = 검색을 하는 필드
		$this->{$this->modelname}->allow_order_field = array('member.mem_id', 'mem_userid', 'mem_username', 'mem_nickname', 'mem_email', 'mem_lastlogin_datetime', 'mem_level'); // 정렬이 가능한 필드
		...
		$company_result = $this->Post_model->get_company_idx($mem_userid);
		$company_idx = $company_result['list'][0]['company_idx'];
		$where['member.company_idx'] = $company_idx;
		 ...
		 */
		$search_option = array('company_name' => '기업명','mem_userid' => '회원아이디', 'mem_email' => '이메일', 'mem_username' => '회원명', 'mem_nickname' => '닉네임', 'mem_register_datetime' => '회원가입날짜', 'mem_register_ip' => '회원가입IP', 'mem_lastlogin_datetime' => '최종로그인날짜', 'mem_lastlogin_ip' => '최종로그인IP', 'mem_adminmemo' => '관리자메모');
	...
	public function write($pid = 0)
	{
        ...
        $getdata['company_list'] = $this->Company_info_model->get_company_list();
		$sql = "select company_name from cb_company_info where company_idx = '".$getdata['company_idx']."'";
		$r = $this->db->query($sql);
		$rowdata = $r->row_array();
		$getdata['company_name'] = $rowdata['company_name'];
		$sql = "select mgr_id from cb_member_group_member where mem_id = '".$pid."'";
		$r = $this->db->query($sql);
		$rowdata = $r->result_array();
		$getdata['member_group'] = $rowdata;
		...
		if ($form_validation === false OR $file_error !== '' OR $file_error2 !== '') {
            ...
            $mem_cur_seed = $this->input->post('mem_cur_seed') ? $this->input->post('mem_cur_seed') : 0;
            $mem_cur_fruit = $this->input->post('mem_cur_fruit') ? $this->input->post('mem_cur_fruit') : 0;
            $mem_ranking = $this->input->post('mem_ranking') ? $this->input->post('mem_ranking') : 0;

			$updatedata = array(
                'mem_userid' => $this->input->post('mem_userid', null, ''),
                'company_idx' => $this->input->post('company_idx', null, ''),
                'mem_div' => $this->input->post('mem_div', null, ''),
                'mem_position' => $this->input->post('mem_position', null, ''),
                'mem_state' => $this->input->post('mem_state', null, ''),
                'mem_cur_seed' => $mem_cur_seed,
				'mem_cur_fruit' => $mem_cur_fruit,
                'mem_ranking' => $mem_ranking,
	...
	public function listdelete()
	{
		 ...
		 * 체크한 게시물의 삭제를 실행합니다
		 */
		if ($this->input->get('chk') && is_array($this->input->get('chk'))) {
			foreach ($this->input->get('chk') as $val) {
				if ($val) {
					$this->member->delete_member($val);
					$this->Member_model->delete_member_userid($val);
				}
			}
		}
	...
	public function excel_upload()
	{
		...
		$layoutconfig = array('layout' => 'layout', 'skin' => 'excel_upload', 'page_title' => '회원추가 엑셀 업로드', 'page_name' => "회원추가 엑셀 업로드");
		$view['layout'] = $this->managelayout->admin($layoutconfig, $this->cbconfig->get_device_view_type());
		$this->data = $view;
		$this->layout = element('layout_skin_file', element('layout', $view));
		$this->view = element('view_skin_file', element('layout', $view));
