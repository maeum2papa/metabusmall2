...
class Fruits extends CB_Controller
{
	 ...
	 * 페이지 이동시 필요한 정보입니다
	 */
	public $pagedir = 'cinfo/fruits';

	/**
	 * 모델을 로딩합니다
	 */
	protected $models = array('Fruit', 'Company_info','Member_meta', 'Member_group', 'Member_group_member', 'Member_nickname', 'Member_extra_vars', 'Member_userid', 'Social_meta', 'Post');
	...
	public function write()
	{
		 ...
		 * 페이지네이션을 생성합니다
		 */
		$config['base_url'] = '/admin/cinfo/fruits/write' . '?' . $param->replace('page');
		$config['total_rows'] = $result['total_rows'];
		$config['per_page'] = $per_page;
		$this->pagination->initialize($config);
		$view['view']['paging'] = $this->pagination->create_links();
		...
		if($this->input->post('mode') == 'register'){
			...
			if ($this->input->post('chk') && is_array($this->input->post('chk'))) {
				...
				redirect('/admin/cinfo/fruits/write');
			} else {
