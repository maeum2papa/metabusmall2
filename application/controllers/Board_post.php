...
class Board_post extends CB_Controller
{
	 ...
	 * 모델을 로딩합니다
	 */
	protected $models = array('Post', 'Post_meta', 'Post_extra_vars', 'Post_file', 'Post_link', 'Board', 'Board_category');
    ...
    public $mem_admin_flag = 0;

	function __construct()
	{
		parent::__construct();
        $this->mem_admin_flag = $this->session->userdata['mem_admin_flag'];
	...
	public function lists($brd_key = '')
	{
		...
		$userdata = array();
		$userdata['mem_is_admin'] = $this->member->item('mem_is_admin');
		$userdata['mem_level'] = $this->member->item('mem_level');
		$userdata['mem_userid'] = $this->member->item('mem_userid');
	...
	public function post($post_id = 0, $print = false)
	{
		...
		// 로그인한 회원정보
		$post['userdata'] = $this->session->userdata();

		$view['view']['post'] = $post;

		$view['view']['reply_list'] = array();
		$view['view']['reply_list'] = $this->Post_model->get_comment_list(element('post_num', $view['view']['post']));
		foreach(element('list', $view['view']['reply_list']) as $key => $val){
			$view['view']['reply_list']['list'][$key]['post_display_name'] = display_username(
				element('post_userid', $val),
				element('post_nickname', $val)
			);
			$view['view']['reply_list']['list'][$key]['board'] = $board2 = $this->board->item_all(element('brd_id', $val));
			if ($board2) {
				$view['view']['reply_list']['list'][$key]['boardurl'] = board_url(element('brd_key', $board2));
				$view['view']['reply_list']['list'][$key]['posturl'] = post_url(element('brd_key', $board2), element('post_id', $val));
			}
			$view['view']['reply_list']['list'][$key]['category'] = '';
			if(element('post_category', $val)){
				$view['view']['reply_list']['list'][$key]['category'] = $this->Board_category_model->get_category_info(element('brd_id', $val), element('post_category', $val));
			}
			if (element('post_image', $val)) {
				...
				$view['view']['reply_list']['list'][$key]['thumb_url'] = thumb_url('post', element('pfi_filename', $file), 80);
			} else {
				$view['view']['reply_list']['list'][$key]['thumb_url'] = get_post_image_url(element('post_content', $val), 80);
			}

			$link_player2 = '';
			$view['view']['reply_list']['list'][$key]['link'] = $link2 = array();
			...
			if (element('post_link_count', $val)) {
				...
				$view['view']['reply_list']['list'][$key]['link'] = $link2 = $this->Post_link_model
					->get('', '', $linkwhere, '', '', 'pln_id', 'ASC');
				if ($link2 && is_array($link2)) {
					foreach ($link2 as $key2 => $value2) {
						$view['view']['reply_list']['list'][$key]['link'][$key2]['link_link'] = site_url('postact/link/' . element('pln_id', $value2));
					}
				}
			}
			$view['view']['reply_list']['list'][$key]['link_count'] = $link_count2 = count($link2);
			...
			if (element('post_file', $val) OR element('post_image', $val)) {
				...
				$view['view']['reply_list']['list'][$key]['file'] = $file2 = $this->Post_file_model
					->get('', '', $filewhere, '', '', 'pfi_id', 'ASC');
				$view['view']['reply_list']['list'][$key]['file_download'] = array();
				$view['view']['reply_list']['list'][$key]['file_image'] = array();
				...
				if ($file2 && is_array($file2)) {
					foreach ($file2 as $key2 => $value2) {
						if (element('pfi_is_image', $value2)) {
							...
							$view['view']['reply_list']['list'][$key]['file_image'][] = $value2;
						} else {
							$value2['download_link'] = site_url('postact/download/' . element('pfi_id', $value2));
							$view['view']['reply_list']['list'][$key]['file_download'][] = $value2;
				...
				$view['view']['reply_list']['list'][$key]['file_count'] = count($file);
				$view['view']['reply_list']['list'][$key]['file_download_count'] = count($view['view']['file_download']);
				$view['view']['reply_list']['list'][$key]['file_image_count'] = count($view['view']['file_image']);
			}
		...
		if (element('post_secret', $post)) {
			if (element('mem_id', $post)) {
				if(element('brd_id', $post) == '4'){
					if($is_admin === false && $this->member->item('mem_level') === 1 && $mem_id !== abs(element('mem_id', $post))){
		...
		if (element('use_personal', $board) && $is_admin === false
			&& $mem_id !== abs(element('mem_id', $post)) && $this->mem_admin_flag == 0 ) {
		...
		if ($use_prev_next) {
			...
			if (element('use_personal', $board) && $is_admin === false && $this->mem_admin_flag == 0) {
		...
		if ($show_list_from_view) {
			...
			// 231220 asmo lkt 포스트에서 리스트 미리보기 관련 변수 추가 (관리자 > 게시판관리 > 목룍페이지 뷰페이지 미리보기 체크해제해야 함)
			$view['view']['listPreview'] = $list = $this->_get_list(element('brd_key', $board), 1);
		}
	...
	public function _get_list($brd_key, $from_view = '')
	{
		...
		// );
		if($this->board->item_key('brd_id', $brd_key) == '3' || $this->board->item_key('brd_id', $brd_key) == '4'){
			$where['post.post_reply'] = '';
		} 
		$where['post.post_del <>'] = 2;
		if (element('except_notice', $board)
			&& $this->cbconfig->get_device_view_type() !== 'mobile') {
			$where['post.post_notice'] = 0;
		}
		if (element('mobile_except_notice', $board)
			&& $this->cbconfig->get_device_view_type() === 'mobile') {
			$where['post.post_notice'] = 0;
		...
		// 로그인한 회원정보
		$userdata = array();
		$userdata['company_idx'] = $this->member->item('company_idx');
		$userdata['mem_is_admin'] = $this->member->item('mem_is_admin');
		$userdata['mem_level'] = $this->member->item('mem_level');
		$userdata['mem_userid'] = $this->member->item('mem_userid');

		if($this->board->item_key('brd_id', $brd_key) == '4'){
			// 문의게시판(기업) 로그인한 회원의 소속값으로 불러오기
			if($userdata['mem_is_admin'] == '0'){ // 최고관리자가 아니면
				if($userdata['mem_level'] == '100'){ // 기업관리자
					$where['company_info.company_idx'] = $userdata['company_idx'];
				} else { // 일반회원
					$where['post.mem_id'] = $mem_id;
					$where['company_info.company_idx'] = $userdata['company_idx'];
				}
			} 
		} else if($this->board->item_key('brd_id', $brd_key) == '3'){
			// 문의게시판(전체) 로그인한 회원이 작성한 글만 
			if($userdata['mem_is_admin'] == '0'){ // 최고관리자가 아니면
				$where['post.mem_id'] = $mem_id;
			}
		} else {
			if($this->board->item_key('brd_id', $brd_key) == '2'){ // 기업 공지사항
				$where['company_info.company_idx'] = $userdata['company_idx']; // 소속 기업의 글만 노출
			} else {
				if (element('use_personal', $board) && $is_admin === false && $this->mem_admin_flag == 0) {
					$where['post.mem_id'] = $mem_id;
				}
		...
		if (element('list', $result)) {
			foreach (element('list', $result) as $key => $val) {
				...
				// 게시판 별, 관리자 등급 별 게시글 경로 변경
				if($this->board->item_key('brd_id', $brd_key) == '3'){ // 문의게시판(전체)
					if($userdata['mem_is_admin'] == '0'){ // 사용자, 기업관리자
						$result['list'][$key]['post_url'] = post_url(element('brd_key', $board), element('post_id', $val));
					} else { // 최고관리자
						//$result['list'][$key]['post_url'] = '/admin/board/qna/post/'.element('post_id', $val);
						$result['list'][$key]['post_url'] = 'javascript:void(0)';
					}
				...
				} else if($this->board->item_key('brd_id', $brd_key) == '4'){ // 문의게시판(기업)
					if($userdata['mem_is_admin'] == '0'){
						if($userdata['mem_level'] == '100'){ // 기업관리자
							$result['list'][$key]['post_url'] = 'javascript:void(0)';
							//$result['list'][$key]['post_url'] = '/admin/board/cqna/post/'.element('post_id', $val);
						} else { // 사용자
							$result['list'][$key]['post_url'] = post_url(element('brd_key', $board), element('post_id', $val));
						}
					} else { // 최고관리자
