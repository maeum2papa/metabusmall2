 ...
 * 세움랭킹
 */
if ( ! function_exists('seum_rank')) {
	function seum_rank($company_idx='',$mem_id=''){
		$CI =& get_instance();
		$q = "set @rank = 0";
		$r = $CI->db->query($q);
		$q = "set @prev_cnt = NULL";
		$r = $CI->db->query($q);
		//$q = "select g.mem_id, m.mem_username, m.mem_nickname, count(g.field_idx) as cnt from cb_game_field as g left join cb_member as m on m.mem_id = g.mem_id where m.mem_id <> '' and m.company_idx = '".$company_idx."' and g.crop_flag = 1 group by g.mem_id  order by cnt desc";
		$q = "SELECT mem_id, mem_username, mem_nickname, cnt, @rank := IF(@prev_cnt = cnt, @rank, @rank + 1) AS rank, @prev_cnt := cnt FROM (SELECT g.mem_id, m.mem_username, m.mem_nickname, COUNT(g.field_idx) AS cnt FROM cb_game_field AS g LEFT JOIN cb_member AS m ON m.mem_id = g.mem_id WHERE m.mem_id <> '' AND m.company_idx = '".$company_idx."' AND g.crop_flag = 1 GROUP BY g.mem_id, m.mem_username, m.mem_nickname ) AS counts ORDER BY cnt DESC";
		$r = $CI->db->query($q);
		$rank = $r->result_array(); 
		foreach ($rank as $k => $v) {
			if($v['mem_id'] == $mem_id){
				$return["my_rank"] = $v['rank'];
			}
			$v[num] = $k+1;
			$return["list"][] = $v;
		}
 ...
 * 클래스룸 상단 메뉴
 */
if ( ! function_exists('seum_menu')) {
	function seum_menu(){ 
		$CI =& get_instance();
		$q = "select * from cb_category where cca_parent = '0' order by cca_order";
		$r = $CI->db->query($q);
		$menu0 = $r->result_array(); 
		foreach ($menu0 as $k => $v) {
			$seum_menu[0][$v[cca_id]][men_id] = $v[cca_id];
			$seum_menu[0][$v[cca_id]][men_parent] = 0;
			$seum_menu[0][$v[cca_id]][men_name] = $v[cca_value];
			$seum_menu[0][$v[cca_id]][men_link] = '';
			$seum_menu[0][$v[cca_id]][men_target] = '';
			$seum_menu[0][$v[cca_id]][men_desktop] = 1;
			$seum_menu[0][$v[cca_id]][men_mobile] = 1;
			$seum_menu[0][$v[cca_id]][men_custom] = '';
			$seum_menu[0][$v[cca_id]][men_order] = $v[cca_order];
			
			$q = "select * from cb_category where cca_parent = '".$v[cca_id]."' order by cca_order";
			$r = $CI->db->query($q);
			$menu1 = $r->result_array(); 
			foreach ($menu1 as $k1 => $v1) {
				$seum_menu[$v[cca_id]][$v1[cca_id]][men_id] = $v1[cca_id];
				$seum_menu[$v[cca_id]][$v1[cca_id]][men_parent] = $v[cca_id];
				$seum_menu[$v[cca_id]][$v1[cca_id]][men_name] = $v1[cca_value];
				$seum_menu[$v[cca_id]][$v1[cca_id]][men_link] = '/classroom/business_class?menu='.$v1[cca_value];
				$seum_menu[$v[cca_id]][$v1[cca_id]][men_target] = '';
				$seum_menu[$v[cca_id]][$v1[cca_id]][men_desktop] = 1;
				$seum_menu[$v[cca_id]][$v1[cca_id]][men_mobile] = 1;
				$seum_menu[$v[cca_id]][$v1[cca_id]][men_custom] = '';
				$seum_menu[$v[cca_id]][$v1[cca_id]][men_order] = $v1[cca_order];
			}
		}
		return $seum_menu;
	}
 ...
 * 학습이 가능한지 확인
 */
if ( ! function_exists('business_studyYn')) {
	function business_studyYn($p_sno='',$company_idx=''){ //p_sno 프로젝트의 번호 , $company_idx 우리회사의 회사번호
		$CI =& get_instance();
 ...
 * 기업명 찾기
 */
if ( ! function_exists('busiNm')) {
	function busiNm($company_idx){
		$CI =& get_instance();
		$q = "select company_name from cb_company_info where company_idx = '".$company_idx."'";
		$r = $CI->db->query($q);
		$company_name = (array) $r->row();
 ...
 * 기업아이콘 찾기
 */
if ( ! function_exists('busiIcon')) {
	function busiIcon($company_idx){
		$CI =& get_instance();
		$q = "select company_logo from cb_company_info where company_idx = '".$company_idx."'";
		$r = $CI->db->query($q);
		$company_logo = (array) $r->row();
		if($company_logo['company_logo']){
			return $company_logo['company_logo'];
		} else {
			return '/uploads/banner/2023/11/6e15a2d067af088c1bc340cc72de4760.svg';
		}
		// if(!$company_logo['company_logo'] || $company_logo['company_logo'] = NULL){
		// 	$company_logo['company_logo'] = '/uploads/banner/2023/11/6e15a2d067af088c1bc340cc72de4760.svg';
		// }
		// return $company_logo[company_logo];
	}
}
...
if ( ! function_exists('debug')) {
	function debug($data=''){
		...
		echo "<xmp style='background:black; color:#43db20; font-size:11px; line-height:16px; text-align:left; position:relative; z-index:1000; padding-left:70px;'>";
		print_r($data);
		echo "</xmp>";
	}
}
...
if ( ! function_exists('upload_mkdir')) {
    function upload_mkdir($upload_path)
    {
