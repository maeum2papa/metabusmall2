...
class Board extends CI_Controller
{
	...
	public function latest($config)
	{
        ...
        $this->CI->db->select("post.*");
		$this->CI->db->from('post');
        $this->CI->db->join('member', 'post.mem_id = member.mem_id', 'left');
        $this->CI->db->join('company_info', 'company_info.company_idx = member.company_idx', 'left');
        $this->CI->db->join("(select post_id, pev_value from cb_post_extra_vars where pev_key = 'view_eday') as post_extra_vars", "post.post_id = post_extra_vars.post_id", 'left');

        $company_idx = $this->CI->session->userdata['company_idx'];

        $def_where = " (case when post_extra_vars.pev_value is not null and post_extra_vars.pev_value != '' then post_extra_vars.pev_value  >= DATE_FORMAT(NOW(),'%m/%d/%Y') else 1=1 end) ";

        if($company_idx != 0) $def_where .= " and cb_member.company_idx in (0,". $company_idx.") ";
        if($this->CI->member->is_member() === false) $def_where .= " and (select bmt_value from cb_board_meta where bmt_key = 'access_list' and brd_id = cb_post.brd_id ) = '' ";

        $this->CI->db->where($def_where, null, false);
